...
function buildMessageHTML(messageText, messageSenderId, messageDateSent, attachmentFileId, messageId, status){
  ...
  var image = messageSenderId.id
  var img = new Image(); 
  img.src = "/static/images/" + image; 

  if (!img.complete) {
    image = "no-user-image.gif"
  }

  var messageHtml =
			'<div class="list-group-item" id="'+messageId+'">'+
        '<img style=" float: left; width: 40px; margin-right: 10px; " src="/static/images/' + image + '" class="img-circle"></img>' +
				'<time datetime="'+messageDateSent+ '" class="pull-right">'
					+jQuery.timeago(messageDateSent)+
				'</time>'+

				'<h4 class="list-group-item-heading">'+messageSenderId.full_name+'</h4>'+
...
function buildDialogHtml(dialogId, dialogUnreadMessagesCount, dialogIcon, dialogName, dialogLastMessage, occupants_ids) {
