          ...
          console.error(err);
            ...
            updateDialogsUsersStorage(jQuery.unique(occupantsIds), function(){
                ...
                resDialogs.items.forEach(function(item, i, arr) {
                    ...
                    if(item.occupants_ids != null && item.occupants_ids.indexOf(target) != -1)
                      index = i
                });

                // and trigger the 1st dialog
                triggerDialog(resDialogs.items[index]._id);
...
function showOrUpdateDialogInUI(itemRes, updateHtml) {
  ...
  var dialogIcon = getDialogIcon(itemRes.photo);
  ...
  if (updateHtml === true) {
    ...
    var dialogHtml = buildDialogHtml(dialogId, dialogUnreadMessagesCount, dialogIcon, dialogName, dialogLastMessage, itemRes.occupants_ids);
    $('#dialogs-list').append(dialogHtml);
	}
}

// add photo to dialogs
function getDialogIcon (photo) {
  // var groupPhoto = '<img src="/static/chat/images/ava-group.svg" width="30" height="30" class="round">';
  var privatPhoto  = '<img src="/static/images/message.png" width="30" height="30" class="round">';
  // var defaultPhoto = '<span class="glyphicon glyphicon-eye-close"></span>';

  /*if(photo != "" && photo != null)
    privatPhoto = '<img src="/static/images/' + photo + '" width="30" height="30" class="round">';
*/
  return privatPhoto;
}
...
// Choose dialog
function triggerDialog(dialogId, flag){
    mutex = false
    // deselect
    ...
    retrieveChatMessages(dialogs[dialogId], null, 0);
...
function sendMessageText(text, opponentId) {
	var msg = {
...
// create new dialog
function createNewDialog(user, message, fn) {
  var usersIds = [user.id];
  var usersNames = [user.login];
  ...
  QB.chat.dialog.create(params, function(err, createdDialog) {
    if (err) {
	  ...
	  if(message != ""){
		  sendMessageText(message, dialogOccupants[0]);
	  }
...
function joinToNewDialogAndShow(itemDialog) {
  ...
  var dialogIcon = getDialogIcon(itemDialog.photo);
