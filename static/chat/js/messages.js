...
function retrieveChatMessages(dialog, beforeDateSent, remove){
    ...
    QB.chat.message.list(params, function(err, messages) {
      if (messages) {
        ...
        messages.items.forEach(function(item, i, arr) {
          ...
          var messageSender = getUserById(messageSenderId);
          ...
          var messageHtml = buildMessageHTML(messageText, messageSender, messageDateSent, messageAttachmentFileId, messageId);
...
function showMessage(userId, msg, attachmentFileId) {
  ...
  var messageHtml = buildMessageHTML(msg.body, user, new Date(), attachmentFileId, msg.id);
